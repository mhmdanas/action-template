module.exports = (() => {
    var __webpack_modules__ = {
                ...
                return {
                    [Symbol.asyncIterator]: () => ({
                        async next() {
                            ...
                            const e = await o({ method: n, url: c, headers: i })
        ...
        8513: (e, t, r) => {
            ...
            function handleEventHandlers(e, t, r) {
                if (!e.hooks[t]) {
                    e.hooks[t] = []
                }
                e.hooks[t].push(r)
            }
            function receiverOn(e, t, r) {
                if (Array.isArray(t)) {
                    t.forEach((t) => receiverOn(e, t, r))
                    return
                }
                if (c.indexOf(t) === -1) {
                    console.warn(
                ...
                if (t === '*' || t === 'error') {
                    const e = t === '*' ? 'any' : t
                    console.warn(
                        `Using the "${t}" event with the regular Webhooks.on() function is deprecated. Please use the Webhooks.on${
                            e.charAt(0).toUpperCase() + e.slice(1)
                        }() method instead`
                    )
                }
                handleEventHandlers(e, t, r)
            }
            function receiverOnAny(e, t) {
                handleEventHandlers(e, '*', t)
            }
            function receiverOnError(e, t) {
                handleEventHandlers(e, 'error', t)
            }
            function wrapErrorHandler(e, t) {
                ...
                if (r && r.catch) {
                    r.catch((e) => {
                        console.log(
                            'FATAL: Error occured in "error" event handler'
                        )
                        console.log(e)
                    })
                }
            }
            function getHooks(e, t, r) {
                const s = [e.hooks[`${r}.${t}`]]
                s.push(e.hooks[r])
                s.push(e.hooks['*'])
                return [].concat(...s.filter(Boolean))
            }
            function receiverHandle(e, t) {
                const r = e.hooks.error || []
                if (t instanceof Error) {
                    const e = Object.assign(new s([t]), {
                        event: t,
                        errors: [t],
                    })
                    r.forEach((t) => wrapErrorHandler(t, e))
                    return Promise.reject(e)
                }
                if (!t || !t.name) {
                    throw new s(['Event name not passed'])
                }
                if (!t.payload) {
                    throw new s(['Event payload not passed'])
                }
                const o = getHooks(e, t.payload.action, t.name)
                if (o.length === 0) {
                ...
                const i = o.map((r) => {
                    ...
                    if (e.transform) {
                        s = s.then(e.transform)
                    }
                    return s
                        .then((e) => {
                            return r(e)
                        })
                        .catch((e) => n.push(Object.assign(e, { event: t })))
                })
                return Promise.all(i).then(() => {
                    ...
                    const e = new s(n)
                    Object.assign(e, { event: t, errors: n })
                    r.forEach((t) => wrapErrorHandler(t, e))
                    throw e
                })
            }
            function removeListener(e, t, r) {
                if (Array.isArray(t)) {
                    t.forEach((t) => removeListener(e, t, r))
                    return
                }
                if (!e.hooks[t]) {
                    return
            ...
            function createEventHandler(e) {
                ...
                return {
                    on: receiverOn.bind(null, t),
                    onAny: receiverOnAny.bind(null, t),
                    onError: receiverOnError.bind(null, t),
                    removeListener: removeListener.bind(null, t),
                    receive: receiverHandle.bind(null, t),
                }
            }
            function isntWebhook(e, t) {
                if (e.method !== 'POST') {
                    return true
                }
                if (
                    typeof e.url !== 'string' ||
                    e.url.split('?')[0] !== t.path
                ) {
                    return true
                }
                return false
            }
            const a = ['x-github-event', 'x-hub-signature', 'x-github-delivery']
            function getMissingHeaders(e) {
                return a.filter((t) => !(t in e.headers))
            }
            function getPayload(e) {
                if (e.body) return Promise.resolve(e.body)
                return new Promise((t, r) => {
                    let o = ''
                    e.setEncoding('utf8')
                    e.on('error', (e) => r(new s([e])))
                    e.on('data', (e) => (o += e))
                    e.on('end', () => {
                        try {
                            t(JSON.parse(o))
            ...
            function sign(e, t) {
                const { secret: r, algorithm: s } =
                    typeof e === 'string'
                        ? { secret: e, algorithm: u.SHA1 }
                        : { secret: e.secret, algorithm: e.algorithm || u.SHA1 }
                if (!r || !t) {
                    throw new TypeError(
                        '[@octokit/webhooks] secret & payload required'
                    )
                }
                if (!Object.values(u).includes(s)) {
                    throw new TypeError(
                        `[@octokit/webhooks] Algorithm ${s} is not supported. Must be  'sha1' or 'sha256'`
                    )
                }
                t = typeof t === 'string' ? t : toNormalizedJsonString(t)
                return `${s}=${o.createHmac(s, r).update(t).digest('hex')}`
            }
            function toNormalizedJsonString(e) {
                return JSON.stringify(e).replace(/[^\\]\\u[\da-f]{4}/g, (e) => {
                    return e.substr(0, 3) + e.substr(3).toUpperCase()
                })
            }
            const l = (e) => {
                return e.startsWith('sha256=') ? 'sha256' : 'sha1'
            }
            function verify(e, t, r) {
                if (!e || !t || !r) {
                    throw new TypeError(
                        '[@octokit/webhooks] secret, eventPayload & signature required'
                    )
                }
                const s = n.Buffer.from(r)
                const i = l(r)
                const c = n.Buffer.from(sign({ secret: e, algorithm: i }, t))
                if (s.length !== c.length) {
                    return false
                }
                return o.timingSafeEqual(s, c)
            }
            function verifyAndReceive(e, t) {
                const r = verify(e.secret, t.payload, t.signature)
                if (!r) {
                    const r = new Error(
                        '[@octokit/webhooks] signature does not match event payload and secret'
                    )
                    return e.eventHandler.receive(
                        Object.assign(r, { event: t, status: 400 })
                    )
                }
                return e.eventHandler.receive({
                    id: t.id,
            ...
            const p = i.debug('webhooks:receiver')
            function middleware(e, t, r, s) {
                if (isntWebhook(t, { path: e.path })) {
                    if (s) {
                        s()
                        return
                    }
                    p(`ignored: ${t.method} ${t.url}`)
                    r.statusCode = 404
                    r.end('Not found')
                    return
                }
                const o = getMissingHeaders(t).join(', ')
                if (o) {
                    const t = new Error(
                        `[@octokit/webhooks] Required headers missing: ${o}`
                    )
                    return e.eventHandler.receive(t).catch(() => {
                        r.statusCode = 400
                        r.end(t.message)
                    })
                }
                const n = t.headers['x-github-event']
                const i = t.headers['x-hub-signature']
                const c = t.headers['x-hub-signature-256']
                const a = t.headers['x-github-delivery']
                p(`${n} event received (id: ${a})`)
                let u = false
                const l = setTimeout(() => {
                    u = true
                    r.statusCode = 202
                    r.end('still processing\n')
                }, 9e3).unref()
                return getPayload(t)
                    .then((t) => {
                        return verifyAndReceive(e, {
                            id: a,
                            name: n,
                            payload: t,
                            signature: c || i,
                    ...
                    .catch((e) => {
                        clearTimeout(l)
                        if (u) return
                        const t = Array.from(e)[0].status
                        r.statusCode = t || 500
                        r.end(e.toString())
                    })
            }
            function createMiddleware(e) {
                if (!e || !e.secret) {
                    throw new Error(
                ...
                const t = {
                    eventHandler: createEventHandler(e),
                    path: e.path || '/',
                    secret: e.secret,
                    hooks: {},
                }
                const r = middleware.bind(null, t)
                r.on = t.eventHandler.on
                r.removeListener = t.eventHandler.removeListener
                return r
            }
            class Webhooks {
                constructor(e) {
                    ...
                    this.sign = sign.bind(null, e.secret)
                    this.verify = verify.bind(null, e.secret)
                    ...
                    this.middleware = middleware.bind(null, t)
                    this.verifyAndReceive = verifyAndReceive.bind(null, t)
                }
            }
            const d = Webhooks.prototype.constructor
            t.Webhooks = Webhooks
            t.createEventHandler = createEventHandler
            t.createMiddleware = createMiddleware
            t.createWebhooksApi = d
            t.sign = sign
            t.verify = verify
        },
        1231: (e, t, r) => {
            ...
            const o = r(7972)
            const n = (e) =>
                e.replace(/\s+at .*aggregate-error\/index.js:\d+:\d+\)?/g, '')
        ...
        7972: (e, t, r) => {
            ...
            const s = r(2087)
            const o = /\s+at.*(?:\(|\s)(.*)\)?/
            const n = /^(?:(?:(?:node|(?:internal\/[\w/]*|.*node_modules\/(?:babel-polyfill|pirates)\/.*)?\w+)\.js:\d+:\d+)|native)/
            const i = typeof s.homedir === 'undefined' ? '' : s.homedir()
            e.exports = (e, t) => {
                t = Object.assign({ pretty: false }, t)
                return e
                    .replace(/\\/g, '/')
                    .split('\n')
                    .filter((e) => {
                    ...
                    .map((e) => {
                        if (t.pretty) {
                            return e.replace(o, (e, t) =>
                                e.replace(t, t.replace(i, '~'))
                            )
                        }
                        return e
                    })
                ...
                function createDebug(e) {
                    ...
                    function debug(...e) {
                        ...
                        e[0] = e[0].replace(/%([a-zA-Z%])/g, (t, s) => {
                            ...
                            n++
                            const o = createDebug.formatters[s]
                    ...
                    Object.defineProperty(debug, 'enabled', {
                        enumerable: true,
                        configurable: false,
                        get: () => (r === null ? createDebug.enabled(e) : r),
                        set: (e) => {
                            r = e
                        },
                    })
        ...
        900: (e) => {
            ...
            function fmtLong(e) {
                var n = Math.abs(e)
                if (n >= o) {
                    return plural(e, n, o, 'day')
                }
                if (n >= s) {
                    return plural(e, n, s, 'hour')
                }
                if (n >= r) {
                    return plural(e, n, r, 'minute')
                }
                if (n >= t) {
                    return plural(e, n, t, 'second')
                }
                return e + ' ms'
            }
            function plural(e, t, r, s) {
                var o = t >= r * 1.5
                return Math.round(e / r) + ' ' + s + (o ? 's' : '')
            }
        },
        467: (e, t, r) => {
            'use strict'
            ...
            var s = _interopDefault(r(2413))
            var o = _interopDefault(r(8605))
            var n = _interopDefault(r(8835))
            var i = _interopDefault(r(7211))
            var c = _interopDefault(r(8761))
            const a = s.Readable
            ...
            const _ = n.parse
            const v = n.format
            const O = 'destroy' in s.Readable.prototype
            ...
            function getNodeRequestOptions(e) {
                ...
                if (e.signal && e.body instanceof s.Readable && !O) {
                    throw new Error(
        ...
        7354: function (e, t, r) {
                        ...
                        r[t] =
                            ...
                            function (r) {
                                return new Promise(function (s, o) {
                                    ;(r = e[t](r)),
                                        settle(s, o, r.done, r.value)
                                })
                            }
                    }
                    function settle(e, t, r, s) {
            ...
            r(8513)
            function getConfig() {
                const e = a.getInput('type')
                if (e !== 'comment' && e !== 'close') {
                    throw Error('type is not comment or close')
                }
                const t = a.getInput('repo-token', { required: true })
                const r = parseInt(a.getInput('days-until-close'))
                if (isNaN(r)) {
                    throw Error('daysUntilClose is not an integer')
                }
                const s = a.getInput('template-not-used-label')
                const o = a.getInput('template-not-used-comment-body')
                const n = a.getInput('doesnt-follow-template-label')
                const i = a.getInput('doesnt-follow-template-comment-body')
                return {
                    type: e,
                    daysUntilClose: r,
                    templateNotUsedLabel: s,
            ...
            class Processor {
                constructor(e, t, r) {
                    a.info('In Processor constructor')
                    this.config = e !== null && e !== void 0 ? e : getConfig()
                    this.octokit = u.getOctokit(this.config.repoToken)
                    this.repo = t !== null && t !== void 0 ? t : l.context.repo
                    if (r !== undefined) {
                        this.payload = r
                    } else if (this.config.type === 'comment') {
                        this.payload = {
                            issue: Object.assign(Object.assign({}, this.repo), {
                                issue_number: l.context.issue.number,
                            }),
                            labelName: l.context.payload.label.name,
                        }
                    }
                }
                createComment(e, t) {
                    return s(this, void 0, void 0, function* () {
                        yield this.octokit.issues.createComment(
                            Object.assign(Object.assign({}, e), { body: t })
                        )
                    })
                }
                getAuthorLogin(e) {
                    var t
                    return s(this, void 0, void 0, function* () {
                        const r = (yield this.octokit.issues.get(
                            Object.assign({}, e)
                        )).data
                        if (r === undefined) {
                ...
                getEvents(e, t) {
                    return s(this, void 0, void 0, function* () {
                        return (yield this.octokit.issues.listEvents(
                            Object.assign(Object.assign({}, e), {
                                per_page: 30,
                                page: t,
                            })
                ...
                getIssues(e) {
                    return c(this, arguments, function* getIssues_1() {
                        let t = 1
                        while (true) {
                            const r = (yield o(
                ...
                processIssue(e, t) {
                    return s(this, void 0, void 0, function* () {
                        a.info('In Processor#processIssue')
                        let r = 1
                        for (;;) {
                            const s = yield this.getEvents(e, r)
                            if (s.length === 0) break
                            let o
                            for (let e = s.length - 1; e >= 0; e--) {
                                const r = s[e]
                                if (r.event !== 'labeled') continue
                                if (r.label.name !== t) continue
                                o = r
                                a.info(`Found event ${r.id}`)
                                break
                            }
                            if (o === undefined) continue
                            const n = new Date(o.created_at)
                            a.info(`event date is ${n}`)
                            const i = new Date()
                            a.info(`Current date is ${i}`)
                            if (
                                i.valueOf() - n.valueOf() >
                                864e5 * this.config.daysUntilClose
                            ) {
                                yield this.closeIssue(e)
                            }
                            r++
                        }
                    })
                }
                closeInvalidIssues(e) {
                    var t, r
                    return s(this, void 0, void 0, function* () {
                        a.info('In Processor#closeInvalidIssues')
                        try {
                            for (
                                var s = n(this.getIssues(e)), o;
                                (o = yield s.next()), !o.done;

                            ) {
                                const t = o.value
                                yield this.processIssue(t, e)
                            }
                        } catch (e) {
                            t = { error: e }
                        } finally {
                            try {
                                if (o && !o.done && (r = s.return))
                                    yield r.call(s)
                            } finally {
                                if (t) throw t.error
                            }
                ...
                interpolateValues(e, t) {
                    return s(this, void 0, void 0, function* () {
                        const r = yield this.getAuthorLogin(t)
                        e = e.replace('{authorLogin}', r)
                        const s = this.config.daysUntilClose
                        let o
                        if (s === 1) {
                            o = '1 day'
                        } else {
                ...
                run() {
                    return s(this, void 0, void 0, function* () {
                        switch (this.config.type) {
                            case 'close':
                                yield this.closeInvalidIssues(
                                    this.config.doesntFollowTemplateLabel
                                )
                                yield this.closeInvalidIssues(
                                    this.config.templateNotUsedLabel
                                )
                                break
                            case 'comment': {
                                ...
                                const e = this.payload.issue
                                const t = this.payload.labelName
                                a.info(`label name is ${t}`)
                                let r
                                switch (t) {
                                    case this.config.templateNotUsedLabel:
                                        a.info(
                                            'label is template-not-used-label'
                                        )
                                        r = this.config
                                    ...
                                    case this.config.doesntFollowTemplateLabel:
                                        a.info(
                                            'label is doesnt-follow-template-label'
                                        )
                                        r = this.config
                                ...
                                yield this.createComment(
                                    e,
                                    yield this.interpolateValues(r, e)
                                )
                                break
                            }
                        }
